...
import net.yacy.grid.tools.Compression;
                ...
                if (mode == COMPRESSED_MODE) {
                    ...
                    if (d.startsWith(prefix) && d.endsWith(".txt")) {
                        ...
                        new Thread() {
                            public void run() {
                                try {
                                    Log.logger.info("starting gzip of " + source);
                                    Compression.gzip(source, dest, true);
                                    Log.logger.info("finished gzip of " + source);
                                } catch (IOException e) {
                                    Log.logger.info("gzip of " + source + " failed: " + e.getMessage());
                                }
                            }
                        }.start();
                    }
