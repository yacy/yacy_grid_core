...
import net.yacy.grid.geo.OSMTile;
...
import net.yacy.grid.graphics.RasterPlotter.FilterMode;
...
/**
 * Draw a map with a position marker inside.
 * http://localhost:8010/yacy/grid/core/vis/map.png?text=Test&mlat=1.28373&mlon=103.84379&zoom=18
 */
public class MapService extends ObjectAPIHandler implements APIHandler {

    private static final long serialVersionUID = -9112326721290824443L;

    @Override
    public String getAPIPath() {
        return "/yacy/grid/core/vis/map.png";
    }
    ...
    public ServiceResponse serviceImpl(Query post, HttpServletResponse response) throws APIException {
        ...
        if (zoom > 19) throw new APIException(503, "zoom must be <= 19");
        double lat = post.get("mlat", 50.11362d);
        if (lat < -90.0) throw new APIException(503, "mlat must be >= -90.0");
        if (lat >  90.0) throw new APIException(503, "mlat must be <= 90.0");
        double lon = post.get("mlon", 8.67919d);
        if (lon < -180.0) throw new APIException(503, "mlon must be >= -180.0");
        if (lon >  180.0) throw new APIException(503, "mlon must be <= 180.0");
        int width = post.get("width", 512);
        if (width > 2560) throw new APIException(503, "width must be <= -2560");
        int height = post.get("height", 256);
        if (height > 2560) throw new APIException(503, "height must be <= -2560");
        ...
        PrintTool.print(map, map.getWidth() - 6, map.getHeight() - 6, 0, "MADE WITH YACY GRID", 1, false, 50);
        ...
        try {
            BufferedImage logo = ImageIO.read(FileSystems.getDefault().getPath("html").resolve("artwork").resolve("marker-red.png").toFile());
